...
export async function sendFixpackReady(opts: {
  ...
  await send({
    ...
    attachments,
    text: [
      `Thanks for buying the fix pack for ${site}.`,
      '',
      ...(pack
        ? [
            `The ${pack.entries.length} files are attached to this email. Start with botready-fixes.md: it is a prompt, so paste it into Claude Code or Cursor in the repository that serves your site and it applies the rest one commit at a time.`,
            '',
            `Prefer one download? ${zipLink}`,
            '',
            `Your result, where you can re-run the check after each change: ${link}`,
            '',
            'Re-run it after each change rather than after all of them. The only proof a client can read your site is that it did.',
          ]
        : [
            'The files did not generate on our side, which is our problem and not yours. Reply to this email and we will send them within the day.',
            '',
            `Your result: ${link}`,
          ]),
      '',
